...
const parseIncomingFiles = require('./utils/parseIncomingFiles');
...
function parseCsvToJsonMultipleFiles(filesPaths) {
  return new Promise((resolve) => {
    const requestedParsing = filesPaths.map((filePath) =>
      parseCsvToJsonSingleFile(filePath),
    );

    Promise.all(requestedParsing).then(function (parsedFiles) {
...
app.post('/upload_files', urlencodedParser, function (req, res) {
  parseIncomingFiles(req)
    .then((relocatedFilesPaths) =>
      parseCsvToJsonMultipleFiles(relocatedFilesPaths),
    )
